...
def graph_colouring_fair_graph(dict_id2tx, dict_id2rx, cue_num, d2d_num, rb_num, dict_id2channel, dict_id2channel_mmwave,
    ...
    produce_cell_interference(list_nodes, dict_id2channel, dict_id2tx, pow(10, 5 / 10), pow(10,0.5))  # 构建蜂窝干扰图
    produce_mmwave_interference_graph(list_nodes, dict_id2channel_mmwave, dict_id2tx, dict_id2rx, pow(10, 0.5)) # 构建毫米波干扰图
    ...
    while (len(dict_colored_node) != d2d_num):
        ...
        for color in node_with_biggest_sinr_or_samllest_inter_power:
            if color != rb_num - 1:
                ...
                dict_chosen_node_rate.append((color, node, 2.16 * pow(10, 6) * math.log2(1 + pow(10, sinr / 10))))
        dict_chosen_node_rate = sorted(dict_chosen_node_rate, key=lambda x: x[2], reverse=True)
        if len(dict_chosen_node_rate) == 0:
            print("error")
            dict_cannot_colored_id2node = {}
            for node in list_nodes:
                if node.rx_id != 0 and node.rx_id not in dict_colored_node:  # 获取所有无法着色的节点   有问题
                    for color in range(rb_num):
                        node.candidate_color.append(color)
        else:
            ...
            if color_id != rb_num - 1: # 蜂窝频段
                ...
                for node in dict_rb[color_id]:  # 计算加入着色节点的信噪比
                    sinr_new = comp_rate_cell(node, dict_rb[color_id], dict_id2tx, dict_id2rx, dict_id2channel, list_nodes, color_id)
                    if pow(10, sinr_new / 10) < 15 and color_id in node.candidate_color:
                        node.candidate_color.remove(color_id)  # 如果有节点不能满足最低信噪比要求， 此节点不能着色
                    sum_rate_new += math.log2(1 + pow(10, sinr_new / 10))
                if sum_rate_new >= sum_rate_old:  # 如果过加入新着色节点后信噪比提升
                    if color_id in node.candidate_color:  # 如果所有用户的信噪比满足条件，则对节点着色
                        node.set_color(color_id)
                        dict_colored_node[node.get_node_id()] = node  # 在已着色字典中加入着色节点
                        dict_id2rx[node.rx_id].set_allocated_rb(color_id)  # 为接收机分配资源块
                        dict_id2tx[node.tx_id].set_allocated_rb(color_id)  # 为发射机分配资源块
                        node.delete_candidate_color()  # 删除节点的候选颜色集
                        for node1 in list_nodes:
                            if node1 != node and node1.rx_id != 0:  # V2V 节点
                                if node1.rx_id in node.cannot_bear_node:  # node_cell 的邻居节点
                                    if node.color in node1.candidate_color:  # 删除node_cell 邻居节点候选颜色集中，node_cell已着颜色
                                        node1.candidate_color.remove(node.color)
                                if node1.rx_id in node.cannot_bear_node_mmWave:
                                    node1.cannot_bear_node_mmWave.remove(
                                        node.get_node_id())  # 在毫米波干扰图中， 删除 node_cell与其邻居节点的干扰边
                    else:  # 如果新加入节点后，有节点信噪比不能满足最低信噪比约束
                        dict_rb[color_id].remove(node_colored)
                else:
                    dict_rb[color_id].remove(node_colored)
                    if color_id in node.candidate_color:
                        node.candidate_color.remove(color_id)
            else:  # 毫米波频段
                for node in dict_rb[color_id]:
                ...
                for node in dict_rb[color_id]:
                    ...
                    if pow(10, sinr_new / 10) < 0.1 and color_id in node.candidate_color:
                        node.candidate_color.remove(color_id)
                    sum_rate_new += math.log2(1 + pow(10, sinr_new / 10))
                if sum_rate_new >= sum_rate_old:
                    if color_id in node.candidate_color:  # 如果所有用户的信噪比满足条件，则对节点着色
                        ...
                        for node1 in list_nodes:
                            if node1 != node and node1.rx_id != 0:  # V2V 节点
                                if node1.rx_id in node.cannot_bear_node_mmWave:  # node_cell 的邻居节点
                                    if node.color in node1.candidate_color:  # 删除node_cell 邻居节点候选颜色集中，node_cell已着颜色
                                        node1.candidate_color.remove(node.color)
                                if node1.rx_id in node.cannot_bear_node:
...
def choose_node_cell_inter_power(dict_rb_v_color):
    ...
    dict_node_id2inter_power = sorted(dict_node_id2inter_power, key=lambda x: x[1])
...
def choose_node_mmwave_inter_power(dict_rb_v_color):
    dict_node_id2inter_power = []
    for node in dict_rb_v_color:
        node_power = 0
